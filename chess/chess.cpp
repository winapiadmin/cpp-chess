...
Outcome::Outcome(std::optional<Color> winner) : winner(winner) {}
...
{
    os << "Outcome(winner=" << (outcome.winner == std::nullopt ? "std::nullopt" : *outcome.winner ? "true"
            : "false")
       << ")";
    return os;
}
...
#else
    // Fallback for compilers without __builtin_ctzll
    if (bb == 0) return -1;
    int index = 0;
    while ((bb & 1) == 0)
    {
...
std::vector<Square> scan_forward(Bitboard bb)
{
    ...
    while (bb)
    {
        int square = lsb(bb); // Use optimized lsb
...
Bitboard BaseBoard::pin_mask(Color color, Square square) const
{
    ...
    for (auto [attacks, sliders] :
            {
                std::make_tuple(BB_FILE_ATTACKS, this->rooks | this->queens),
...
bool Board::is_valid() const
{
    ...
    */
    ...
    if (!(this->occupied_co[WHITE] & this->kings))return false;
    if (!(this->occupied_co[BLACK] & this->kings))return false;
    if (popcount(this->occupied & this->kings) > 2)return false;

    // Pawns can not be on the back rank.
    if (this->pawns & BB_BACKRANKS)return false;
    // Castling rights.
    if (this->castling_rights != this->clean_castling_rights())return false;
    // En passant.
    std::optional<Square> valid_ep_square = this->_valid_ep_square();
    if (this->ep_square != valid_ep_square)return false;

    // Side to move giving check.
    if (this->was_into_check())return false;
